...
int SerialPort::Set(int flow_ctrl,int databits,int stopbits,char parity){
...
int SerialPort::writeBuffer(double angle, double distance, int flag) {
    std::cout<<angle<<"    "<<distance<<"    "<<flag<<std::endl;
    data send;
    send.angle=angle;
    send.distance=distance;
    send.flag=flag;
    unsigned char chs[100]="";
    memcpy(chs,&send, sizeof(send));
